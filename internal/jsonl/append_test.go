...
func TestAppendLineDurableCheckedRunsBeforeWrite(t *testing.T) {
	path := filepath.Join(t.TempDir(), "ledger.jsonl")
	if err := AppendLineDurableChecked(path, []byte(`{"ok":true}`), 0o644, func() error {
		return errors.New("reject write")
	}); err == nil || !strings.Contains(err.Error(), "reject write") {
		t.Fatalf("check error = %v", err)
	}
	if _, err := os.Stat(path); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("rejected write must not create the file, stat err=%v", err)
	}
	if err := AppendLineDurableChecked(path, []byte(`{"ok":true}`), 0o644, func() error { return nil }); err != nil {
