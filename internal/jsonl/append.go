...
func AppendLine(path string, body []byte, perm os.FileMode) error {
	return appendLine(path, body, perm, false, 0, nil)
}
...
func AppendLineWithin(path string, body []byte, perm os.FileMode, timeout time.Duration) error {
	return appendLine(path, body, perm, false, timeout, nil)
}
...
func AppendLineDurable(path string, body []byte, perm os.FileMode) error {
	return appendLine(path, body, perm, true, 0, nil)
}

// AppendLineDurableChecked holds the file lock, runs check, then appends. Use
// it when the caller must inspect current file contents before committing the
// new row (for example launch-ledger write-time transition gates).
func AppendLineDurableChecked(path string, body []byte, perm os.FileMode, check func() error) error {
	if check == nil {
		return errors.New("jsonl: AppendLineDurableChecked requires a check")
	}
	return appendLine(path, body, perm, true, 0, check)
}

func appendLine(path string, body []byte, perm os.FileMode, durable bool, timeout time.Duration, check func() error) error {
	if path == "" {
